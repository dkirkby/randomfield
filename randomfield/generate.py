...
class Generator(object):
    ...
    """
    def __init__(self, nx, ny, nz, spacing, cosmology=None, power=None,
                 verbose=False):
        ...
        self.k_min, self.k_max = powertools.get_k_bounds(
            self.plan_c2r.data_in, spacing, packed=True)

        if cosmology is None:
            self.cosmology = cosmotools.create_cosmology()
            if power is None:
                self.power = powertools.load_default_power()
            else:
                powertools.validate_power(power)
                self.power = power
        else:
            self.cosmology = cosmology
            if power is None:
                self.power = cosmotools.calculate_power(
        ...
        self.redshifts = cosmotools.get_redshifts(
            ...
            spacing=self.spacing, scaled_by_h=True, z_axis=2).reshape(nz)
        ...
        self.angular_spacing[1:] = (
            (180 / np.pi) * (self.spacing / self.cosmology.h) / DA[1:])
        ...
        if self.verbose:
            ...
            print('{0} Mpc/h spacing covered by k = {1:.5f} - {2:.5f} h/Mpc.'
                  .format(spacing, self.k_min, self.k_max))
    ...
    def generate_delta_field(self, smoothing_length=0., seed=None):
        """
        ...
        power = powertools.filter_power(self.power, smoothing_length)
        powertools.tabulate_sigmas(self.plan_c2r.data_in, power=self.power,
